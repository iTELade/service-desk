# Aktualizacja istniejącej instalacji do 0.7.1
...
Prześlij **Service_Desk_0.7.1.zip** na serwer. Polecenia zakładają dotychczasową instalację `/opt/itelade-desk` i nazwany wolumen `/app/data`.

```bash
unzip -t Service_Desk_0.7.1.zip
mkdir -p desk-update-0.7.1
unzip Service_Desk_0.7.1.zip -d desk-update-0.7.1
bash /home/dehmead/desk-update-0.7.1/itelade-desk/scripts/upgrade.sh /opt/itelade-desk
```
...
5. Podmienia pliki aplikacji, uruchamia 0.7.1 i sprawdza `/healthz`: wersja 0.7.1, schemat 7.
...
Nie wymaga `docker compose down` i nie usuwa wolumenu. Zatrzymanie oraz migracja oznaczają przerwę w działaniu WWW. Kopie powstają w `backups/before-0.7.1-<czas>/`. Zapewnij miejsce na nowy obraz, kopię danych i kodu. Nie kasuj ich przed sprawdzeniem działania.
...
**Service_Desk_0.7.1.zip jest pełną paczką**, nie nakładką. Rozpakuj ją do osobnego katalogu i uruchom jej `scripts/upgrade.sh`. Skrypt obsługuje istniejące schematy 4, 5, 6 i 7. Zachowuje `.env`, nazwy kontenera i wolumenu oraz wykonuje kopię danych przed migracją.
...
```bash
docker compose -f compose.yaml -f backups/before-0.7.1-TUTAJ_CZAS/rollback.yaml up -d --no-build --no-deps desk
```
