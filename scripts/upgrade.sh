...
desk_backup="$desk_live/backups/before-0.8.0-$desk_stamp"
mkdir -p "$desk_backup"
...
echo 'Buduję 0.8.0 przy zachowaniu obecnego kontenera.'
docker build -t service-desk:0.8.0 "$desk_source"
docker run --rm --network none --entrypoint node service-desk:0.8.0 -e 'import("./lib/version.mjs").then(x=>{if(x.VERSION!=="0.8.0")process.exit(1)})'
desk_rollback=0
desk_data_ready=0
...
docker run --rm --network none --user 0 --entrypoint node -v "$desk_backup/data:/backup" service-desk:0.8.0 -e 'const {DatabaseSync}=require("node:sqlite"),fs=require("fs");const d=new DatabaseSync("/backup/desk.sqlite");if(d.prepare("PRAGMA integrity_check").get().integrity_check!=="ok")throw Error("Kopia uszkodzona");const v=d.prepare("PRAGMA user_version").get().user_version;if(![4,5,6,7,8].includes(v))throw Error("Wymagany schemat 4, 5, 6, 7 lub 8");d.close();if(fs.readFileSync("/backup/master.key").length!==32)throw Error("Nieprawidłowy master.key")'
desk_data_ready=1
...
docker tag service-desk:0.8.0 service-desk:current
docker compose config --quiet
docker compose up -d --no-build --no-deps desk
desk_ok=0
for ((desk_try=0;desk_try<60;desk_try++)); do
  if docker exec "$desk_name" node -e 'fetch("http://127.0.0.1:3000/healthz",{signal:AbortSignal.timeout(3000)}).then(async r=>{const x=await r.json();if(!r.ok||x.version!=="0.8.0"||x.schema!==8)process.exit(1)}).catch(()=>process.exit(1))' >/dev/null 2>&1; then desk_ok=1; break; fi
  sleep 2
done
[ "$desk_ok" = 1 ]
desk_rollback=0
trap - ERR INT TERM
printf '\nAktualizacja do 0.8.0 zakończona. Kopia: %s\n' "$desk_backup"
