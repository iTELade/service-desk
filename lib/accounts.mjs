...
export function createAccounts(db,{origin,encodePassword,dataDir,transport:injectedTransport,env=process.env}){
  ...
  let sending=false;
  const settings=()=>{const r=db.prepare('SELECT * FROM app_settings WHERE id=1').get();return {...JSON.parse(r.config),version:r.version,smtp_configured:smtp.settings().configured};};
  const publicConfig=()=>{const s=settings();return {brand_name:s.brand_name,registration_mode:s.registration_mode,password_reset_available:smtp.settings().configured,email_registration_available:smtp.settings().configured};};
  function save(b){
    ...
    db.prepare('UPDATE app_settings SET config=?,version=version+1 WHERE id=1').run(JSON.stringify({...JSON.parse(db.prepare('SELECT config FROM app_settings WHERE id=1').get().config),brand_name:brand,company_name:text(b.company_name??old.company_name??brand,'Nazwa organizacji',2,100),registration_mode:mode,allowed_domains:domains}));
    return settings();
  }
