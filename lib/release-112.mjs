...
function safeColumns(value){
  ...
  for(const key of DEFAULT_COLUMNS)if(!columns.includes(key))columns.push(key);
  return columns;
}
...
export function createRelease112(db,projects,desk){
  ...
  function preferences(method,user,query,b){
    ...
    const columns=safeColumns(b.columns),sortPrimary=SORTS.has(String(b.sort_primary||''))?String(b.sort_primary):'updated_desc',sortSecondary=b.sort_secondary&&SORTS.has(String(b.sort_secondary))?String(b.sort_secondary):'',quick=QUICK_FILTERS.has(String(b.quick_filter||''))?String(b.quick_filter):'';
