...
export function normalizeEntry(entry,c){
  ...
  const adminByGroup=(c.global_admin_group_dns||[]).some(v=>groups.includes(String(v).trim().toLowerCase()));
  const adminByUser=(c.global_admin_users||[]).some(v=>{const x=String(v).trim().toLowerCase();return x===login||x===mail;});
  const role=adminByGroup||adminByUser?'admin':c.default_role==='agent'||[...memberships.values()].some(r=>r!=='requester')?'agent':'customer';
  return {uid,login,email:mail,name,dn,active,role,is_internal:c.is_internal,groups,memberships:[...memberships].map(([project_id,role])=>({project_id,role}))};
}
export function createDirectory(db,projects,{dataDir,clientFactory}={}){
  ...
  function validate(b){
    ...
    if(!Array.isArray(c.global_admin_group_dns)||c.global_admin_group_dns.length>100)fail(400,'Maksymalnie 100 grup administratorów LDAP.');
    c.global_admin_group_dns=[...new Set(c.global_admin_group_dns.map(v=>text(String(v),'DN grupy administratorów',1,4096)))];
    if(!Array.isArray(c.global_admin_users)||c.global_admin_users.length>100)fail(400,'Maksymalnie 100 użytkowników administratorów LDAP.');
    c.global_admin_users=[...new Set(c.global_admin_users.map(v=>text(String(v),'Login lub e-mail administratora',1,254).toLowerCase()))];
  ...
  function commitRows(rows,c,summary,user){
    ...
    return tx(()=>{
      ...
      for(const r of rows){
        ...
        if(db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='v8_user_directory_groups'").get()){db.prepare('DELETE FROM v8_user_directory_groups WHERE user_id=?').run(u.id);if(r.active)for(const group of r.groups)db.prepare('INSERT OR IGNORE INTO v8_user_directory_groups(user_id,group_dn) VALUES(?,?)').run(u.id,group);}
