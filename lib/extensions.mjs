...
export function createExtensions(db,projects,catalog,workflows,desk,accounts,identity,sso,options){
  ...
  function events(){if(maintenance())return;if(processing)return;processing=true;try{
    for(let n=0;n<500;n++){
      ...
      const errors=[];
      for(const [name,run] of [['sync',sync.process],['mail',mail.notify],['webhooks',webhooks.notify],['comments',commentActions],['notifications',notifications.notify]]){
  ...
  async function handle(method,path,user,query,read){if(!path.startsWith('/api/desk/'))return null;const b=method==='GET'?{}:await read();user=db.prepare('SELECT * FROM users WHERE id=?').get(user.id);if(!user?.active||!user.directory_active||user.registration_state!=='active'||user.account_kind!=='human')fail(401,'Sesja wygasła.');const parts=path.slice('/api/desk/'.length).split('/'),[resource,id,action]=parts,pid=id?Number(id):null;let value,status=200;
    if(resource==='api-tokens')value=method==='GET'?api.list(user):action==='revoke'?api.revoke(pid,user):api.create(b,user);
    else if(resource==='system'){if(user.role!=='admin')fail(403,'Wymagany administrator.');value={version:VERSION,schema_version:SCHEMA_VERSION,node:process.version,uptime_seconds:Math.floor(process.uptime()),mail_queued:db.prepare("SELECT COUNT(*) n FROM mail_outbox WHERE status='queued'").get().n,mail_failed:db.prepare("SELECT COUNT(*) n FROM mail_outbox WHERE status='failed'").get().n,events_failed:db.prepare("SELECT COUNT(*) n FROM desk_events WHERE status='failed'").get().n};}
    ...
    }else fail(404,'Nieznany moduł.');events();return {status,value};
  }
  return {api,lifecycle,templates,notifications,sync,resources,mail,webhooks,events,handle,start(){timer=setInterval(events,1000).unref();mail.start();webhooks.start();},stop(){clearInterval(timer);mail.stop();webhooks.stop();}};
}
