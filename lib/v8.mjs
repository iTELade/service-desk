...
import {now,fail,text,email,integer,boolean,txFor} from './core.mjs';
...
export function createV8(db,{projects,desk,workflows,catalog,accounts,dataDir,origin,fetcher=fetch}){
  ...
  function saveGithub(id,b,u){
    admin(u);
    const p=projects.requireProject(integer(Number(b.project_id),'Project'),u,true),form=db.prepare('SELECT * FROM request_types WHERE id=? AND project_id=? AND enabled=1').get(integer(Number(b.request_type_id),'Request type'),p.id),reporter=githubReporters().find(x=>x.id===integer(Number(b.reporter_id),'Reporter'));
  ...
  async function pollGithubIntegration(c){
    ...
    try{
      ...
      for(const issue of issues){if(issue.pull_request)continue;if(db.prepare('SELECT id FROM v8_github_links WHERE integration_id=? AND issue_id=?').get(c.id,issue.id))continue;
        let t;try{t=await createGithubTicket(c,issue);}catch(e){pending++;errors.push('Ticket #'+issue.number+': '+String(e.message));continue;}
        imported++;const id=Number(db.prepare('INSERT INTO v8_github_links(integration_id,issue_id,issue_number,issue_url,ticket_id,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').run(c.id,issue.id,issue.number,issue.html_url,t.id,'ticket_created',now(),now()).lastInsertRowid);
