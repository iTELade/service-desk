...
export function createV8(db,{projects,desk,workflows,catalog,dataDir,origin}){
  const tx=txFor(db),secrets=secretStore(dataDir);
  let timer=null,busy=false;

  const parse=(v,fallback)=>{try{return JSON.parse(v);}catch{return fallback;}};
  const role=(id)=>db.prepare('SELECT * FROM v8_roles WHERE id=?').get(id);
  const admin=u=>{if(u.role!=='admin')fail(403,'Administrator required.');};
  const staff=u=>{if(!['admin','agent'].includes(u.role))fail(403,'Team access required.');};

  function effectivePermissions(u){
    if(u.role==='admin')return [...ALL];
    const ids=new Set(db.prepare('SELECT role_id FROM v8_user_roles WHERE user_id=?').all(u.id).map(x=>x.role_id));
    for(const row of db.prepare(`SELECT gr.role_id FROM v8_group_roles gr
      JOIN v8_user_directory_groups ug ON lower(ug.group_dn)=lower(gr.group_dn)
      WHERE ug.user_id=?`).all(u.id))ids.add(row.role_id);
    const permissions=new Set();
    for(const id of ids){
      const r=role(id);if(!r)continue;
      for(const p of parse(r.permissions,[])){
        if(p==='*')for(const x of ALL)permissions.add(x);else if(ALL.has(p))permissions.add(p);
      }
    }
    return [...permissions];
  }
  function requirePermission(u,p){
    ...
    if(!effectivePermissions(u).includes(p))fail(403,'Missing permission: '+p);
  }
  ...
  function roles(u){
    ...
    const rows=db.prepare('SELECT * FROM v8_roles ORDER BY protected DESC,name').all();
    return rows.map(r=>({...r,permissions:parse(r.permissions,[]),
      users:db.prepare('SELECT user_id FROM v8_user_roles WHERE role_id=?').all(r.id).map(x=>x.user_id),
      groups:db.prepare('SELECT group_dn FROM v8_group_roles WHERE role_id=? ORDER BY group_dn').all(r.id).map(x=>x.group_dn)
    }));
  }
  function saveRole(id,b,u){
    requirePermission(u,'users.manage');
    const name=text(b.name,'Role name',2,100);
    if(!Array.isArray(b.permissions)||b.permissions.some(p=>p!=='*'&&!ALL.has(p)))fail(400,'Invalid permissions.');
    if(!Array.isArray(b.users)||!Array.isArray(b.groups))fail(400,'Invalid assignments.');
    const old=id?role(id):null;if(id&&!old)fail(404,'Role not found.');if(old?.protected&&b.delete)fail(409,'Protected role cannot be deleted.');
    return tx(()=>{
      if(b.delete){db.prepare('DELETE FROM v8_roles WHERE id=?').run(id);audit(u,'rbac.role_deleted',{role_id:id,name});return {ok:true};}
      if(old)db.prepare('UPDATE v8_roles SET name=?,permissions=?,version=version+1,updated_at=? WHERE id=?').run(name,JSON.stringify([...new Set(b.permissions)]),now(),id);
      else id=Number(db.prepare('INSERT INTO v8_roles(name,permissions,created_at,updated_at) VALUES(?,?,?,?)').run(name,JSON.stringify([...new Set(b.permissions)]),now(),now()).lastInsertRowid);
      db.prepare('DELETE FROM v8_user_roles WHERE role_id=?').run(id);db.prepare('DELETE FROM v8_group_roles WHERE role_id=?').run(id);
      for(const uid of new Set(b.users.map(Number))){
        if(!db.prepare('SELECT id FROM users WHERE id=?').get(uid))fail(400,'Unknown user.');
        db.prepare('INSERT INTO v8_user_roles VALUES(?,?)').run(uid,id);
      }
      for(const group of new Set(b.groups.map(x=>text(String(x),'Group DN',1,4096))))db.prepare('INSERT INTO v8_group_roles VALUES(?,?)').run(group,id);
      audit(u,'rbac.role_saved',{role_id:id,name,permissions:b.permissions,user_count:b.users.length,group_count:b.groups.length});
      return roles(u).find(x=>x.id===id);
    });
  }
  ...
  function assetList(u){
    requirePermission(u,'assets.manage');
  ...
  function saveAssetMeta(id,b,u){
    requirePermission(u,'assets.manage');const a=db.prepare('SELECT * FROM assets WHERE id=?').get(integer(id,'Asset'));if(!a)fail(404,'Asset not found.');
  ...
  function relation(b,u){
    requirePermission(u,'assets.manage');const parent=integer(Number(b.parent_asset_id),'Parent asset'),child=integer(Number(b.child_asset_id),'Child asset');
  ...
  function githubList(u){
    requirePermission(u,'integrations.manage');
    return db.prepare('SELECT id,name,owner,repo,project_id,request_type_id,reporter_id,enabled,auto_close,comment_template,label_priority,poll_minutes,last_poll,last_error,version,created_at,updated_at FROM v8_github_integrations ORDER BY id').all().map(r=>({...r,label_priority:parse(r.label_priority,{})}));
  }
  function saveGithub(id,b,u){
    requirePermission(u,'integrations.manage');const p=projects.requireProject(integer(Number(b.project_id),'Project'),u,true),form=db.prepare('SELECT * FROM request_types WHERE id=? AND project_id=?').get(integer(Number(b.request_type_id),'Request type'),p.id),reporter=db.prepare('SELECT * FROM users WHERE id=?').get(integer(Number(b.reporter_id),'Reporter'));
    if(!form||!reporter?.active)fail(400,'Invalid request type or reporter.');
    const old=id?db.prepare('SELECT * FROM v8_github_integrations WHERE id=?').get(id):null;if(id&&!old)fail(404,'Integration not found.');
    if(b.delete){db.prepare('UPDATE v8_github_integrations SET enabled=0,version=version+1,updated_at=? WHERE id=?').run(now(),id);return {ok:true};}
    ...
    if(old)db.prepare(`UPDATE v8_github_integrations SET name=?,owner=?,repo=?,project_id=?,request_type_id=?,reporter_id=?,token_secret=?,enabled=?,auto_close=?,comment_template=?,label_priority=?,poll_minutes=?,version=version+1,updated_at=? WHERE id=?`)
      .run(text(b.name,'Name',2,100),text(b.owner,'Owner',1,100),text(b.repo,'Repository',1,100),p.id,form.id,reporter.id,token,Number(Boolean(b.enabled)),Number(Boolean(b.auto_close)),tpl,JSON.stringify(priorities),integer(Number(b.poll_minutes||5),'Poll minutes',1,60),now(),id);
    else id=Number(db.prepare(`INSERT INTO v8_github_integrations(name,owner,repo,project_id,request_type_id,reporter_id,token_secret,enabled,auto_close,comment_template,label_priority,poll_minutes,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(text(b.name,'Name',2,100),text(b.owner,'Owner',1,100),text(b.repo,'Repository',1,100),p.id,form.id,reporter.id,token,Number(Boolean(b.enabled)),Number(Boolean(b.auto_close)),tpl,JSON.stringify(priorities),integer(Number(b.poll_minutes||5),'Poll minutes',1,60),now(),now()).lastInsertRowid);
    audit(u,'github.integration_saved',{integration_id:id,repository:`${b.owner}/${b.repo}`});return githubList(u).find(x=>x.id===id);
  }
  async function githubRequest(c,path,options={}){
    const token=secrets.open(c.token_secret),res=await fetch('https://api.github.com'+path,{...options,headers:{Accept:'application/vnd.github+json','User-Agent':'iTELade-Service-Desk/0.8.0','X-GitHub-Api-Version':'2022-11-28',Authorization:'Bearer '+token,'Content-Type':'application/json',...(options.headers||{})},signal:AbortSignal.timeout(10000)});
    const body=await res.text();if(!res.ok)throw new Error(`GitHub ${res.status}: ${body.slice(0,180)}`);return body?JSON.parse(body):{};
  }
  function githubPriority(c,issue){
    const map=parse(c.label_priority,{});for(const l of issue.labels||[]){const name=typeof l==='string'?l:l.name;if(map[name]&&['P1','P2','P3','P4'].includes(map[name]))return map[name];}return 'P3';
  }
  function createGithubTicket(c,issue){
    ...
    const description=`GitHub issue: ${issue.html_url}\nAuthor: @${issue.user?.login||'unknown'}\n\n${issue.body||''}`.slice(0,20000);
    ...
    return tx(()=>{db.prepare('UPDATE projects SET next_number=next_number+1,version=version+1 WHERE id=?').run(p.id);
      const id=Number(db.prepare(`INSERT INTO tickets(project_id,key,number,title,description,type,priority,status,workflow_status,reporter_id,created_by,created_at,updated_at,request_type_id,form_snapshot,custom_values,origin)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'github')`).run(p.id,projects.issueKey(p,n),n,prepared.title,prepared.description,prepared.type,prepared.priority,initial.category,initial.key,actor.id,actor.id,stamp,stamp,prepared.id,prepared.snapshot,prepared.values).lastInsertRowid);
      const t=desk.ticket(id);workflows.run(t,'ticket_created',actor);projects.audit(p.id,actor,'github.issue_imported',{ticket_id:id,issue_number:issue.number,repository:`${c.owner}/${c.repo}`});return t;});
  }
  async function finalizeGithub(c,link){
    ...
    if(c.auto_close)await githubRequest(c,`/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/issues/${link.issue_number}`,{method:'PATCH',body:JSON.stringify({state:'closed'})});
  ...
  async function pollGithubIntegration(c){
    const run=Number(db.prepare('INSERT INTO v8_github_runs(integration_id,status,detail,created_at) VALUES(?,?,?,?)').run(c.id,'running','Polling '+c.owner+'/'+c.repo,now()).lastInsertRowid);
    try{
      for(const link of db.prepare("SELECT * FROM v8_github_links WHERE integration_id=? AND status<>'done' ORDER BY id LIMIT 20").all(c.id)){try{await finalizeGithub(c,link);}catch(e){db.prepare("UPDATE v8_github_links SET last_error=?,updated_at=? WHERE id=?").run(String(e.message).slice(0,500),now(),link.id);}}
      ...
      for(const issue of issues){if(issue.pull_request)continue;if(db.prepare('SELECT id FROM v8_github_links WHERE integration_id=? AND issue_id=?').get(c.id,issue.id))continue;
        const t=createGithubTicket(c,issue),id=Number(db.prepare('INSERT INTO v8_github_links(integration_id,issue_id,issue_number,issue_url,ticket_id,status,created_at,updated_at) VALUES(?,?,?,?,?,"ticket_created",?,?)').run(c.id,issue.id,issue.number,issue.html_url,t.id,now(),now()).lastInsertRowid);
        try{await finalizeGithub(c,db.prepare('SELECT * FROM v8_github_links WHERE id=?').get(id));}catch(e){db.prepare('UPDATE v8_github_links SET last_error=?,updated_at=? WHERE id=?').run(String(e.message).slice(0,500),now(),id);}
      }
      db.prepare('UPDATE v8_github_integrations SET last_poll=?,last_error=NULL WHERE id=?').run(Date.now(),c.id);db.prepare('UPDATE v8_github_runs SET status=?,detail=? WHERE id=?').run('success','Poll completed',run);
  ...
  async function handle(method,path,u,query,read){
    ...
    if(resource==='summary'){staff(u);value={version:'0.8.0',permissions:effectivePermissions(u),dashboard:dashboard(u),features:{rbac:true,sla:true,inbound_email:true,webhooks:true,audit:true,saved_views:true,approvals:true,organizations:true,assets:true,github:true}};}
    else if(resource==='roles')value=method==='GET'?roles(u):saveRole(id,b,u);
    else if(resource==='permissions')value={permissions:effectivePermissions(u)};
    else if(resource==='views')value=method==='GET'?listViews(u):saveView(id,b,u);
    else if(resource==='dashboard')value=dashboard(u);
    else if(resource==='audit'){value=auditRows(u,query);if(query.get('format')==='csv'){const esc=x=>`"${String(x??'').replaceAll('"','""')}"`;value={content_type:'text/csv',filename:'service-desk-audit.csv',data:['id,created_at,actor,project,action,details',...value.map(r=>[r.id,r.created_at,r.actor_name||'',r.project_key||'',r.action,JSON.stringify(r.details)].map(esc).join(','))].join('\n')};}}
    ...
    else if(resource==='github')value=method==='GET'?githubList(u):saveGithub(id,b,u);
    else if(resource==='github-runs'){requirePermission(u,'integrations.manage');value=db.prepare('SELECT * FROM v8_github_runs ORDER BY id DESC LIMIT 100').all();}
    else fail(404,'Unknown 0.8 resource.');
    return {status:200,value};
  }
  return {handle,effectivePermissions,background,start(){timer=setInterval(()=>void background(),15000).unref();void background();},stop(){if(timer)clearInterval(timer);}};
}
