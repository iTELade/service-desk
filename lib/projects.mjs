...
export function createProjects(db){
  ...
  function create(b,user){
    ...
    return tx(()=>{
      ...
      const template=db.prepare('SELECT id,config FROM workflow_templates WHERE is_default=1').get();if(template)db.prepare('UPDATE project_workflows SET config=? WHERE project_id=?').run(template.config,id);if(template)db.prepare('UPDATE projects SET workflow_template_id=? WHERE id=?').run(template.id,id);
      db.prepare("INSERT INTO project_members(project_id,user_id,role) SELECT ?,id,'agent' FROM users WHERE account_kind='service' AND username IN ('desk.bot','itelade.bot')").run(id);
