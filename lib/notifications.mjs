...
export function createNotifications(db,projects,desk){
  ...
  function recipients(t,{internal=false,actor_id=null,mentions=[]}={}){
    const ids=new Set([t.reporter_id,t.assignee_id,...db.prepare('SELECT user_id FROM ticket_watchers WHERE ticket_id=?').all(t.id).map(x=>x.user_id),...mentions]);
  ...
  function notify(e){
    ...
    const payload=JSON.parse(e.payload);let actor=e.event==='ticket_created'?t.created_by:payload.actor_id??t.last_actor_id,internal=false,mentions=[],action=e.event==='ticket_created'?'Utworzono zgłoszenie':e.event==='status_changed'?'Zmieniono status':e.event==='ticket_updated'?'Zaktualizowano zgłoszenie':null;
    if(e.event==='comment_added'){
      const c=db.prepare('SELECT * FROM comments WHERE id=?').get(payload.comment_id);if(!c)return;actor=c.actual_actor_id||c.author_id;internal=Boolean(c.internal);action=internal?'Dodano notatkę wewnętrzną':'Dodano odpowiedź';
      mentions=[...c.body.matchAll(/@([a-zA-Z0-9._-]+)/g)].map(m=>db.prepare('SELECT id FROM users WHERE username=?').get(m[1])?.id).filter(Boolean);
    }
    if(!action)return;
    for(const u of recipients(t,{actor_id:actor,internal,mentions}).filter(staff))db.prepare('INSERT OR IGNORE INTO notifications(user_id,ticket_id,body,event_id,created_at) VALUES(?,?,?,?,?)').run(u.id,t.id,`${action}: ${t.key} — ${t.title}`,e.id,e.created_at);
  }
  ...
  return {recipients,notify,list,seen};
}
