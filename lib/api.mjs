...
export function createApi(db,projects,catalog,workflows,desk){
  ...
  function create(b,u){admin(u);const p=projects.requireProject(integer(b.project_id,'Projekt'),u,true),bot=db.prepare('SELECT * FROM users WHERE id=?').get(integer(b.user_id,'Konto serwisowe'));
    if(p.archived||p.module_type==='assets'||!bot?.active||!bot.directory_active||bot.registration_state!=='active'||bot.account_kind!=='service'||!projects.canWork(bot,p))fail(400,'Wybierz aktywne konto serwisowe obsługujące ten projekt.');
