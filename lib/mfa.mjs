...
export function createMfa(db,{dataDir,clock=Date.now}){
  ...
  function status(u){const r=row(u.id);return {enabled:Boolean(r?.enabled),totp_enabled:Boolean(r?.enabled),recovery_remaining:r?.enabled?JSON.parse(r.recovery).length:0};}
  function setup(u){
    if(row(u.id)?.enabled)fail(409,'Najpierw wyłącz obecną konfigurację TOTP.');
    const key=base32(randomBytes(20));
    db.prepare('INSERT INTO user_mfa(user_id,secret,pending_until) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET secret=excluded.secret,pending_until=excluded.pending_until,last_step=-1').run(u.id,secrets.seal(key),clock()+600000);
    const label='Service Desk:'+String(u.email||'user').slice(0,80);const uri='otpauth://totp/'+encodeURIComponent(label)+'?secret='+key+'&issuer=Service%20Desk&algorithm=SHA1&digits=6&period=30';
    return {secret:key,uri,qr_svg:qrSvg(uri)};
  }
  function consume(id,code,{pending=false}={}){
    const r=row(id);if(!r||(!pending&&!r.enabled)||(pending&&r.pending_until<clock()))return false;code=String(code||'').replace(/[\s-]/g,'');
    ...
    if(!pending&&/^[a-f0-9]{20}$/.test(code)){const codes=JSON.parse(r.recovery),i=codes.indexOf(hash(code));if(i>=0){codes.splice(i,1);db.prepare('UPDATE user_mfa SET recovery=? WHERE user_id=?').run(JSON.stringify(codes),id);return true;}}
    return false;
  }
  function enable(u,code){return tx(()=>{if(row(u.id)?.enabled)fail(409,'TOTP jest już aktywne.');if(!consume(u.id,code,{pending:true}))fail(400,'Nieprawidłowy kod lub wygasła konfiguracja.');const recovery=Array.from({length:10},()=>randomBytes(10).toString('hex'));db.prepare('UPDATE user_mfa SET enabled=1,recovery=?,pending_until=0 WHERE user_id=?').run(JSON.stringify(recovery.map(hash)),u.id);db.prepare('DELETE FROM sessions WHERE user_id=? AND token<>?').run(u.id,u.token||'');audit(u.id,'mfa.enabled');return {recovery_codes:recovery};});}
