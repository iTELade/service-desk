...
export function createTemplates(db,projects,workflows){
  ...
  function save(id,b,u){admin(u);return tx(()=>{
    ...
    const isDefault=b.is_default===undefined?Number(old?.is_default||false):Number(b.is_default===true);if(old?.is_default&&!isDefault&&!db.prepare('SELECT 1 FROM workflow_templates WHERE is_default=1 AND id<>?').get(id))fail(409,'Najpierw oznacz inny szablon jako domyślny.');
  ...
  function remove(id,b,u){admin(u);return tx(()=>{
    ...
    const assigned=uses(id);if(assigned.length)fail(409,'Szablon jest używany przez projekty: '+assigned.map(p=>p.key).join(', ')+'. Najpierw przypisz im inny szablon.');if(t.is_default)fail(409,'Najpierw wybierz inny szablon domyślny.');
  ...
  function detach(projectId,b,u){projects.requireProject(projectId,u,true);fail(409,'Projekt musi używać centralnego szablonu. Przypisz inny szablon.');}
