...
export function createSync(db,projects,desk,workflows){
  ...
  function applyStatus(r,target,key,source){
    ...
    db.prepare("UPDATE tickets SET workflow_status=?,status=?,resolution_text=?,resolved_at=?,updated_at=?,version=version+1,change_source='sync',last_actor_id=? WHERE id=?").run(s.key,s.category,source.resolution_text||target.resolution_text,['resolved','closed'].includes(s.category)?target.resolved_at||now():null,now(),a.id,target.id);
