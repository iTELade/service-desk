...
const actions=['status','comment','priority','assign','lock','unlock','webhook','resolution'];
  ...
  return input.map((raw,index)=>{
    ...
    out.actions=r.actions.map(a=>{
      ...
      if(type==='comment'){
        ...
        const bot_id=a.bot_id??null;if(bot_id!==null){const bot=db.prepare('SELECT * FROM users WHERE id=?').get(integer(bot_id,'Bot'));if(!active(bot)||bot.account_kind!=='service'||!projects.canWork(bot,project))fail(400,'Wybierz aktywne konto serwisowe projektu.');}
...
export function createAutomation(db,projects,getWorkflow,{clock=Date.now}={}){
  ...
  const botFor=(p,id)=>{const u=id?db.prepare('SELECT * FROM users WHERE id=?').get(id):db.prepare("SELECT * FROM users WHERE username IN ('desk.bot','itelade.bot') AND account_kind='service'").get();if(!active(u)||u.account_kind!=='service'||!projects.canWork(u,p))fail(409,'Konto serwisowe nie ma dostępu do projektu.');return u;};
  ...
  function execute(t,r,event,actor){
    ...
    for(const a of r.actions){
      ...
      if(a.type==='status'&&a.value!==t.workflow_status){
        ...
        const bot=botFor(p,a.bot_id);db.prepare("INSERT INTO comments(ticket_id,author_id,body,internal,created_at,automation_rule,actual_actor_id,origin) VALUES(?,?,?,?,?,?,?,'automation')").run(t.id,bot.id,render(a.body,t),a.internal?1:0,iso(),r.id,actorId);
