...
const ui=index.match(/<script data-sd151-ui>([\s\S]*?)<\/script>/)?.[1]||'';
const version=read('lib/version.mjs');

test('1.5.1 version and cache boundary are aligned',()=>{
  assert.match(version,/VERSION='1\.5\.1'/);
  assert.match(version,/SCHEMA_VERSION=8/);
  for(const asset of ['app.css','i18n.js','app.js','settings-center.js','security.js','release-1.1.2.js','release-1.2.0.js'])assert.ok(index.includes(`${asset}?v=1.5.1`),`missing 1.5.1 cache key for ${asset}`);
  assert.ok(css&&live&&ui,'all contained 1.5.1 bootstrap hotfix blocks must exist');
});
...
test('1.5.1 queue live refresh runs before app.js and never calls route',()=>{
