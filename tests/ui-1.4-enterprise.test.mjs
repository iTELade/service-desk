...
test('1.4.3 product boundary is versioned without a schema migration',()=>{
  assert.match(version,/VERSION='1\.4\.3'/);
  assert.match(version,/SCHEMA_VERSION=8/);
  assert.match(index,/\?v=1\.4\.3/);
});
...
test('1.4 keeps all agreed operational feature modules active',()=>{
  ...
  assert.ok(index.includes('/release-1.1.2.js?v=1.4.3'));
});
...
test('1.4.1 premium visual system remains active in 1.4.3',()=>{
